    ...
    function Const() {
        return {
            ...
            WAYBILL_STATUS: {
                STATUS_INIT: 0,
                STATUS_RECEIVING: 1,
                STATUS_RECEIVING_PENDING_TRUCK_LOADING: 2,
                STATUS_RECEIVE_SUCCESS: 3,
                STATUS_IN_TRANSIT: 4,
                STATUS_DELIVERING: 5,
                STATUS_DELIVER_SUCCESS: 6,
                STATUS_RETURNING: 7,
                STATUS_RETURNING_SUCCESS: 8,

                STATUS_CANCEL: -1,
                STATUS_RECEIVE_REJECT: -2,
                STATUS_RECEIVE_FAIL: -3,
                STATUS_DELIVER_REJECT: -4,
                STATUS_DELIVER_FAIL: -5,
                STATUS_FAIL: -500
            },
...
(function () {
    ...
    function WaybillStatus(Core) {
        return function (data) {
            if (data >= 0) {
                return Core.Const.WAYBILL_SUCCESS[parseInt(data)];
            } else {
                return Core.Const.WAYBILL_FAIL[parseInt(data) * -1];
            }
...
(function(){
    angular
        .module('app')
        .config(['$stateProvider', '$urlRouterProvider', '$ocLazyLoadProvider', 'IdleProvider', 'KeepaliveProvider', function(
        ...
        )
        {
            ...
            $stateProvider
                ...
                .state('index', {
                    ...
                    templateUrl: 'launch/index/index.html'
                })
        ...
        .run(['$rootScope', '$state', 'Core', function($rootScope, $state, Core) {
            ...
            var appLaunchInitId = Core.localStorageService.get(Core.Config.DATA.KEY_APP_LAUNCH_INIT_ID);

            var userType = Core.localStorageService.get(Core.Config.DATA.KEY_USER_TYPE);
            var url = ""; 
            switch (parseInt(appLaunchInitId))
            {
                case 1:
                    url = userType == Core.Config.DATA.APP_USER_TYPE.APP_USER_TYPE_USER ? "user.html#//user/home" : "user.html#/login";
                    Core.$window.location.href = url;
                    break;

                case 2:
                    url = userType == Core.Config.DATA.APP_USER_TYPE.APP_USER_TYPE_EXPRESS ? "express.html#/courier/home" : "express.html#/login";
                    Core.$window.location.href = url;
                    break;

                case 3:
                    url = userType == Core.Config.DATA.APP_USER_TYPE.APP_USER_TYPE_ADMIN ? "admin.html#/admin/home" : "admin.html#/login";
                    Core.$window.location.href = url;
...
(function(){
    angular
        .module('app')
        .controller('IndexController', ['$scope', '$uibModal', 'Core', IndexController]);


    function IndexController($scope, $uibModal, Core)
    {
        var vm = $scope;
        ...
        vm.appList = [

            {
                id: 1,
                name: '商户',
                img: './launch/img/app-user-icon.png',
            },
            {
                id: 2,
                name: '物流公司',
                img: './launch/img/app-express-icon.png',
            },
            {
                id: 3,
                name: '管理后台',
                img: './launch/img/app-admin-icon.png',
            }
        ];

        vm.clickApp = clickApp;
        
        function clickApp(id)
        {
            Core.localStorageService.set(Core.Const.DATA.KEY_APP_LAUNCH_INIT_ID, id);
            var url = "index.html";
            switch (parseInt(id))
            {
                case 1:
