...
(function () {
    ...
    function Api($http, $q, Data, Const, Log, Util, Config) {
        var apiList = {
            ...
            Waybill: {
                getWaybillList: ['waybill/waybill-list', 'page', 'filter', 'begin_date', 'end_date', 'sn', 'from_company_name', 'to_company_name', 'status'],
                ...
                getWaybillStatistics: ['waybill/waybill-statistics', 'page', 'begin_date', 'end_date', 'express_company_id', 'express_company_branch_name'],
                createWaybillReturn: ['waybill/waybill-return-create', 'waybill_id', 'return_cod', 'return_charge_back', 'return_charge_go_user_type', 'return_charge_back_user_type', 'return_express_comment'], //创建退货api
                getWaybillReturnList: ['waybill/waybill-return-list', 'page', 'filter', 'begin_date', 'end_date', 'waybill_sn', 'status', 'from_company_name', 'to_company_name', 'return_status'],
                deleteReturnWaybill: ['waybill/waybill-return-delete', 'waybill_id'],
                updateWaybillStatusDeliverSuccess: ['waybill/waybill-deliver-success-update', 'waybill_id'],
            ...
            WaybillCross: {
                ...
                saveWaybillCross: ['waybill-cross-city/waybill-save', 'waybill_id', 'from_address_id', 'to_address_id', 'content', 'comment', 'package', 'amount', 'insure_declare_value', 'insure_charge', 'charge', 'charge_pay_type', 'cod', 'deliver_type', 'flag_send_deliver_success_message', 'express_branch_id', 'express_company_id', 'express_waybill_id'],
            ...
            Express: {
                ...
                getBranchList: ['express/branch-list', 'page', 'province', 'city', 'name'],
                getAllBranchList: ['express/branch-list-all'],
                deleteBranch: ['express/branch-delete', 'express_branch_id'],
                saveBranch: ['express/branch-save', 'express_branch_id', 'name', 'contact', 'phone', 'telephone', 'province', 'city', 'county', 'address', 'status']
            },
            Finance: {
                getTradeListIn: ['finance/trade-list-of-in', 'page', 'begin_date', 'end_date', 'status', 'subject'],
                getTradeListOut: ['finance/trade-list-of-out', 'page', 'begin_date', 'end_date', 'status', 'subject'],
                getTradeDetail: ['finance/trade-detail', 'trade_id']
            },
            Company: {
                //getCompanyDetail: ['company/detail'],
                //saveCompany: ['company/company-update', 'legal_person_name', 'legal_person_phone', 'legal_person_id_no', 'booth_number', 'business_scope', 'province', 'city', 'county', 'address', 'telephone', 'fax'],
                deleteAccount: ['company/admin-remove', 'admin_id'],
                getAccountList: ['company/admin-list'],
                updateAccount: ['company/admin-save', 'admin_id', 'username', 'password', 'name', 'express_branch_id', 'auth_list'],
                addAccount: ['company/child-user-add', 'phone', 'password', 'code']
            },
            BankCard: {
            ...
            Team: {
                getCarList: ['express/car-list'],
                saveCar: ['express/car-save', 'car_id', 'express_branch_id', 'license_plate_number', 'model', 'length', 'purchase_time'],
                deleteCar: ['express/car-remove', 'car_id'],
                getDriverList: ['express/courier-list'],
                saveDriver: ['express/courier-save', 'courier_id', 'express_branch_id', 'name', 'phone', 'id_no', 'id_card_front_img', 'id_card_back_img', 'driver_license_img'],
                deleteDriver: ['express/courier-remove', 'courier_id'],
                getPdaList: ['express/pda-user-list'],
                savePda: ['express/pda-user-save', 'pda_user_id', 'express_branch_id', 'username', 'password', 'name', 'phone', 'gender', 'birthday', 'entry_time', 'status'],
                deletePda: ['express/pda-user-remove', 'pda_user_id']
            }
        };
    ...
    function Const() {
        return {
            ...
            NET: {
                ...
                // END_POINT: 'http://yunto.api/express/100',
                END_POINT: 'http://yex-api.yuntu78.com/express/100',
                VERSION: 1,
                CLIENT: 3,
                SP: 2,
...
(function () {
    angular
        ...
        .filter('pdaStatus', ['Core', pdaStatus])
        .filter('waybillReturnStatus', ['Core', waybillReturnStatus])
        .filter('gender', ['Core', gender])
    ...
    function EditWaybillContent() {
        return function (data) {
            ...
            if (data == 'change_status') return "订单状态";
            if (data == 'change_route') return "网点";
        }
    }
    ...
    function confirmContent()
    {
        return function (type)
        {
            ...
            if (type == 'delete_team_car') return "确定删除该车辆吗？";
            if (type == 'delete_team_driver') return "确定删除该司机吗？";
            if (type == 'delete_team_pda') return "确定删除该PAD人员吗？";
            if (type == 'delete_account') return "确定删除该账号吗？";
            if (type == 'delete_return_waybill') return "确定删除吗?";
            if (type == 'update_waybill_deliver_success') return "确定签收吗?";
            if (type == 'logout') return "确定退出登录吗?";
            if (type == 'delete_express_branch') return "确定删除该网点吗?";
        }
    }
    ...
    function pdaStatus(Core) {
        return function (data) {
            return Core.Const.PDA_STATUS[parseInt(data)];
        }
    }

    function gender(Core) {
        return function (data) {
            return Core.Const.GENDER[parseInt(data)];
        }
    }
...
(function () {
    angular
        ...
        .config(['$stateProvider', '$urlRouterProvider', '$ocLazyLoadProvider', 'IdleProvider', 'KeepaliveProvider', function ($stateProvider, $urlRouterProvider, $ocLazyLoadProvider, IdleProvider, KeepaliveProvider) {
            ...
            $urlRouterProvider.otherwise('/express/home');
            ...
            var expressLayout = 'express/layout/main.html';

            $stateProvider
                .state('login', {
                    url: '/login',
                    controller: 'LoginController',
                    templateUrl: 'express/login/login.html',
                    resolve: {
                ...
                .state('express.home', {
                    ...
                    templateUrl: 'express/home/home.html',
                    resolve: {
                        loadPlugin: function ($ocLazyLoad) {
                            return $ocLazyLoad.load([
                                {
                ...
                .state('team.car', {
                    ...
                    templateUrl: 'express/team/car-list.html',
                })
                .state('team.driver', {
                    url: '/driver',
                    controller: 'DriverListController',
                    templateUrl: 'express/team/driver-list.html',
                })
                .state('team.pda', {
                    url: '/pda',
                    controller: 'PdaListController',
                    templateUrl: 'express/team/pda-list.html',
                })
                ...
                .state('waybill.list', {
                    ...
                    templateUrl: 'express/waybill/waybill-list.html'
                })
                .state('waybill.detail', {
                    ...
                    templateUrl: 'express/waybill/waybill-detail.html'
                })
                .state('waybill.returnlist', {
                    url: '/returnlist',
                    controller: 'WaybillReturnListController',
                    templateUrl: 'express/waybill/waybill-return-list.html'
                })
                .state('waybill.statistics', {
                    url: '/statistics',
                    controller: 'WaybillStatisticsController',
                    templateUrl: 'express/waybill/waybill-statistics.html'
                })
                ...
                .state('finance.payDetail', {
                    url: '/pay-detail/:payId',
                    controller: 'FinancePayDetailController',
                    templateUrl: 'express/finance/finance-pay-detail.html',
                })
                .state('finance.receive', {
                    url: '/receive',
                    controller: 'FinanceReceiveController',
                    templateUrl: 'express/finance/finance-receive.html',
                    resolve: {
                        loadPlugin: function ($ocLazyLoad) {
                            return $ocLazyLoad.load([
                                {
                ...
                .state('finance.pay', {
                    url: '/pay',
                    controller: 'FinancePayController',
                    templateUrl: 'express/finance/finance-pay.html',
                })
                ...
                .state('branch.list', {
                    ...
                    templateUrl: 'express/branch/branch-list.html'
                })
                ...
                .state('account.list', {
                    ...
                    templateUrl: 'express/account/account-list.html'
                })
                ...
                .state('post.detail', {
                    ...
                    templateUrl: 'express/post/post-detail.html'
                })
        ...
        .run(['$rootScope', '$state', 'Core', function ($rootScope, $state, Core) {
            ...
            $rootScope.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
                ...
                if (!(detail2List || list2Detail)) {
                    Core.Data.set("express-waybill-list-search", {});
                }
            });
    ...
    function LayoutController($scope, $templateCache, $interval, Core) {
        ...
        function logout() {
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/public-modal/confirm-modal.html',
...
(function(){
    angular
        .module('app')
        .controller('BranchListController', ['$scope', 'Core', BranchListController]);


    function BranchListController($scope, Core)
    {
        var vm = $scope;
        ...
        function openModalOfEdit(data) {
            data = data ? data : null;
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/branch/edit-branch-modal.html',
...
(function(){
    ...
    function ExpressBranchEditController($scope, $templateCache, $uibModalInstance, Core, RegionControl, data)
    {
        ...
        vm.saveData = {
            ...
            expressCompanyId: data ? data.express_company_id : 0,
            name: data ? data.name : "",
...
(function () {
    ...
    function EditAccountModalController($scope, $templateCache, $uibModalInstance, Core, account) {
        ...
        vm.branchList = [];
        vm.adminId = account ? account.id : 0;
        vm.username = account ? account.username : '';
        vm.name = account ? account.name : '';
        vm.password = account ? account.password : '';
        vm.expressBranchId = account ? account.express_branch_id : '';
        vm.menuList = [
            {value: '2', name: '车队管理', key: 'auth_express_car'},
            {value: '3', name: '订单管理', key: 'auth_waybill'},
            {value: '4', name: '财务管理', key: 'auth_finance'},
            {value: '5', name: '网点管理', key: 'auth_express_branch'},
            {value: '6', name: '账户管理', key: 'auth_system'}
        ];

        if (account) {
            var authData = account.admin_auth ? account.admin_auth.data : null;
        ...
        console.log($templateCache.info() + 'Edit account');
        ...
        function getBranchList() {
            Core.Api.Express.getAllBranchList().then(
                function (data) {
                    vm.branchList = data.express_branch_list;
                    if (vm.expressBranchId) {
                        angular.forEach(vm.branchList, function (data, index, array) {
                            if (data.id == vm.expressBranchId) {
                                vm.branchList.selected = data;
                            }
                        })
                    }
                    Core.$timeout(function () {
        ...
        function save() {
            if (vm.branchList.selected != undefined) {
                vm.expressBranchId = vm.branchList.selected.id;
            }
...
(function(){
    ...
    function AccountListController($scope, Core)
    {
        ...
        function getAccountList() {
            ...
            Core.Api.Company.getAccountList().then(
                function (data) {
                    vm.accountList = data.admin_list;
                },
                function (reason) {
        ...
        function editAccount(account) {
            ...
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/account/edit-account-modal.html',
...
(function(){
    angular
        .module('app')
        .controller('HomeController', ['$scope', '$templateCache', '$interval', 'Core', HomeController]);
...
(function(){
    angular
        .module('app')
        .controller('FinancePayDetailController', ['$scope', '$stateParams', 'Core', FinancePayDetailController]);


    function FinancePayDetailController($scope, $stateParams, Core)
    {
        var vm = $scope;

        var id = $stateParams.payId;
...
(function(){
    angular
        .module('app')
        .controller('FinancePayController', ['$scope', '$templateCache', '$uibModal', 'Core', FinancePayController]);


    function FinancePayController($scope, $templateCache, $uibModal, Core)
    {
        var vm = $scope;
...
(function(){
    angular
        .module('app')
        .controller('FinanceReceiveController', ['$scope', '$templateCache', '$uibModal', 'Core', FinanceReceiveController]);


    function FinanceReceiveController($scope, $templateCache, $uibModal, Core)
    {
        var vm = $scope;
        ...
        function login() {
            ...
            Core.Api.Admin.login(username, password).then(
                function (data) {
                    ...
                    Core.Data.setUserType(Core.Config.DATA.APP_USER_TYPE.APP_USER_TYPE_EXPRESS);
                    
                    Core.$state.go('express.home');
                },
                function (reason) {
                    if (reason.code == 3) {
                        Core.Notify.error("密码错误");
                    } else if (reason.code == 5) {
                        Core.Notify.error('用户不存在');
                    } else {
                        Core.Notify.error(reason.message);
                    }
                }
        ...
        function save() {
            ...
            switch (vm.type)
            {
                ...
                case 'delete_account':
                    ...
                    break;
                case 'delete_team_car':
                    url = Core.Api.Team.deleteCar(id);
                    break;
                case 'delete_team_driver':
                    url = Core.Api.Team.deleteDriver(id);
                    break;
                case 'delete_team_pda':
                    url = Core.Api.Team.deletePda(id);
                    break;
                case "delete_return_waybill":
                    url = Core.Api.Waybill.deleteReturnWaybill(id);
                    break;
                case "update_waybill_deliver_success":
                    url = Core.Api.Waybill.updateWaybillStatusDeliverSuccess(id);
                    vm.message = "签收成功";
                    break;
                case 'logout':
                    url = "";
                    ...
                    break;
                case "delete_express_branch":
                    Core.Api.Express.deleteBranch(id);
                    break;
                default:
            ...
            if(url){
                url.then(
                    function (data) {
                        ...
                        Core.Notify.success(vm.message);
                    },
                    function (reason) {
                        Core.Notify.error('失败原因： ' + reason.message);
                    }
                );
            }
...
(function(){
    angular
        .module('app')
        .controller('CarListController', ['$scope', 'Core', CarListController]);

    function CarListController($scope, Core)
    {
        var vm = $scope;
        ...
        function getCarList() {
            ...
            Core.Api.Team.getCarList().then(
                function (data) {
                    vm.carList = data.car_list;
                },
                function (reason) {
                    Core.Notify.error(reason.message);
                }
        ...
        function editCar(car) {
            car = car ? car : null;
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/team/edit-car-modal.html',
...
(function(){
    angular
        .module('app')
        .controller('DriverListController', ['$scope', 'Core', DriverListController]);

    function DriverListController($scope, Core)
    {
        var vm = $scope;
        ...
        function getDriverList() {
            ...
            Core.Api.Team.getDriverList().then(
                function (data) {
        ...
        function editDriver(driver) {
            ...
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/team/edit-driver-modal.html',
                controller: 'EditDriverModalController',
                resolve: {
                    driver: function () {
                        return driver;
                    }
...
(function(){
    angular
        .module('app')
        .controller('PdaListController', ['$scope', 'Core', PdaListController]);

    function PdaListController($scope, Core)
    {
        var vm = $scope;
        ...
        function getPdaList() {
            ...
            Core.Api.Team.getPdaList().then(
                function (data) {
                    vm.pdaList = data.pda_user_list;
                },
                function (reason) {
                    Core.Notify.error(reason.message);
                }
        ...
        function editPda(pda) {
            pda = pda ? pda : null;
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/team/edit-pda-modal.html',
                controller: 'EditPdaModalController',
                resolve: {
                    pda: function () {
                        return pda;
                    }
...
(function(){
    angular
        .module('app')
        .controller('EditCarModalController', ['$scope', '$templateCache', '$uibModalInstance', 'Core', 'car', EditCarModalController]);


    function EditCarModalController($scope, $templateCache, $uibModalInstance, Core, car)
    {
        var vm = $scope;
        ...
        vm.branchList = [];
        vm.carId = car ? car.id : 0;
        vm.plateNumber = car ? car.license_plate_number : '';
        vm.length = car ? car.length : '';
        vm.model = car ? car.model : '';
        vm.date = {
            purchaseTime : car ? car.purchase_time * 1000 : ''
        };
        vm.expressBranchId = car ? car.express_branch_id : '';
        ...
        console.log($templateCache.info() + 'Edit car');
        ...
        function getBranchList() {
            Core.Api.Express.getAllBranchList().then(
                ...
                function (reason) {
                    Core.Notify.error('获取网点列表失败： ' + reason.message);
                }
            )
        }
        ...
        function save() {
            vm.expressBranchId = vm.branchList.selected.id;
            Core.Api.Team.saveCar(
                vm.carId,
                vm.expressBranchId,
                vm.plateNumber,
                vm.model,
                vm.length,
                vm.date.purchaseTime / 1000
            ).then(
                function (data) {
...
(function(){
    angular
        .module('app')
        .controller('EditDriverModalController', ['$scope', '$templateCache', '$uibModalInstance', 'Core', 'driver', 'Upload', EditDriverModalController]);


    function EditDriverModalController($scope, $templateCache, $uibModalInstance, Core, driver, Upload)
    {
        var vm = $scope;

        vm.title = driver ? '编辑' : '添加';

        vm.branchList = [];
        vm.driverId = driver ? driver.id : 0;
        vm.name = driver ? driver.name : '';
        vm.phone = driver ? driver.phone : '';
        vm.id_no = driver ? driver.id_no : '';
        vm.id_card_front_img = driver ? driver.id_card_front_img : '';
        vm.id_card_back_img = driver ? driver.id_card_back_img : '';
        vm.driver_license_img = driver ? driver.driver_license_img : '';
        vm.expressBranchId = driver ? driver.express_branch_id : '';
        ...
        console.log($templateCache.info() + 'Edit driver');
        ...
        vm.upload = function (file, type) {
           ...
           Upload.upload({
               ...
               switch (type) {
                   case 1:
                       vm.id_card_front_img = vm.imgUrl;
                       break;
                   case 2:
                       vm.id_card_back_img = vm.imgUrl;
                       break;
                   case 3:
                       vm.driver_license_img = vm.imgUrl;
                       break;
                   default:
                       break;
               }
        ...
        function removeImg(type) {
            if (type == 1) {
                vm.id_card_front_img = '';
                vm.img = '';
            } else if (type == 2) {
                vm.id_card_back_img = '';
            } else {
                vm.driver_license_img = '';
            }
        }
...
(function(){
    angular
        .module('app')
        .controller('EditPdaModalController', ['$scope', '$templateCache', '$uibModalInstance', 'Core', 'pda', EditPdaModalController]);

    function EditPdaModalController($scope, $templateCache, $uibModalInstance, Core, pda)
    {
        var vm = $scope;
        ...
        vm.branchList = [];
        vm.pdaId = pda ? pda.id : 0;
        vm.username = pda ? pda.username : '';
        vm.password = '';
        vm.passwordRepeat = '';
        vm.name = pda ? pda.name : '';
        vm.phone = pda ? pda.phone : '';
        vm.gender = pda ? pda.gender : 0;
        vm.status = pda ? pda.status : 0;
        vm.date = {
            birthday : pda ? pda.birthday * 1000 : '',
            entryTime : pda ? pda.entry_time * 1000 : ''
        };
        vm.expressBranchId = pda ? pda.express_branch_id : '';
        ...
        console.log($templateCache.info() + 'Edit pda');
        ...
        function save() {
            ...
            Core.Api.Team.savePda(
                ...
                vm.phone,
                vm.gender,
                vm.date.birthday / 1000,
                vm.date.entryTime / 1000,
                vm.status
            ).then(
                function (data) {
                    Core.Notify.success('保存成功！');
    ...
    function WaybillDetailController($scope, $templateCache, Core, $stateParams) {
        ...
        function getWaybillDetail() {
            ...
            Core.Api.Waybill.getWaybillDetail(vm.waybillId).then(
                ...
                function (reason) {
                    //console.log('fff');
                    Core.Notify.error('获取订单详情失败：' + reason.message);
                }
            )
        }
        ...
        function openEditModal(type, waybill) {
            ...
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/waybill/edit-waybill-modal.html',
...
(function(){
    ...
    function EditWaybillController($scope, $uibModalInstance, Core, type, data)
    {
        ...
        function save() {
            ...
            {
                Core.Api.Waybill.updateWaybillCommentAdmin(data.id, vm.editContent.comment).then(
                    function (response) {
...
(function () {
    angular
        .module('app')
        .controller('WaybillListController', ['$scope', 'Core', WaybillListController]);

    function WaybillListController($scope, Core) {
        var vm = $scope;
        var isFirst = true;
        var cacheSearch = {};
        vm.count = 0;
        ...
        function getCacheSearch() {
            cacheSearch = Core.Data.get("express-waybill-list-search") ? Core.Data.get("express-waybill-list-search") : {};
        ...
        function startSaveCacheSearchService() {
            vm.$watchGroup(['date.start', 'date.end', 'search.waybillSn', 'search.fromCompany', 'search.toCompany', 'statusList.selected', 'oddList.selected', 'payTypeList.selected'], function (newValue, oldValue, scope) {
                ...
                cacheSearch.payTypeListSelected = newValue[7];
                Core.Data.set("express-waybill-list-search", cacheSearch);
            });
        ...
        function showWaybillReturnModal(data) {
            data = data ? data : null;
            var uibModal = Core.$uibModal.open({
                templateUrl: 'express/waybill/waybill-return-modal.html',
...
(function(){
    angular
        .module('app')
        .controller('WaybillStatisticsController', ['$scope', '$element', 'Core', WaybillStatisticsController]);

    function WaybillStatisticsController($scope, $element, Core)
    {
        $element[0].querySelector('#start-date').onchange = function () {
            console.log('start date change');
            console.log(arguments);
        ...
        function getWaybillOfExpress(isExport) {
            ...
            if (isExport)
            {
                ...
                Core.$window.location.href = Core.Api.getUrl('waybill/waybill-statistics-export', data);
            }
            else
            { 
                Core.Api.Waybill.getWaybillStatistics(
                    vm.currentPage,
                    startDate,
                    endDate,
                    expressCompanyId,
